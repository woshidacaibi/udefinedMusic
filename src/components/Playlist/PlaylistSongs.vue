...
import { getListSongs, getListPartSongs } from '@/hooks/playlist';
...
const props = withDefaults(
  defineProps<{ id: string | number; trackCount: number }>(),
  {
    id: '',
    trackCount: 0,
  }
...
async function getAllTracks() {
  if (props.id) {
    ...
    const res = await getListSongs(props.id);
    a.close();
...
watch(
  ...
  async (newVal) => {
    ...
    if (i === -1) {
      const res = await getListPartSongs(
        props.id,
        pageSize,
        (newVal - 1) * pageSize
      );
    ...
    } else currentTrack.value = list.value[i].tracks;
