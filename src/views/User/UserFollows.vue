<template>
  <h2>{{ route.query.nickname ? route.query.nickname + '的关注' : '' }}</h2>
  <div style="margin-bottom: 10px" v-if="isLoad">
    <ul>
    ...
    <no-more-tip v-if="info.length === 0" content="暂无关注"></no-more-tip>
  </div>
...
import { getFollows, getUserInfo } from '@/hooks/user';
...
async function getInfo() {
  ...
  info.value = res.data.follow;
}
async function getTotal() {
  const res = await getUserInfo(route.query.id as string);
  total.value = res.profile.follows ? res.profile.follows : 0;
}
watch(
  () => route.query.id,
  () => {
    if (route.name === 'follows') {
      info.value = [];
      error.value = false;
      pageNum.value = 1;
      total.value = 0;
